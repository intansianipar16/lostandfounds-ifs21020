...
import com.ifs21020.lostandfounds.data.remote.response.LFLoginResponse
...
class AuthRepository private constructor(
    ...
    fun register(name: String, email: String, password: String): Flow<MyResult<LFResponse>> = flow {
        ...
        try {
            ...
            emit(
                MyResult.Error(
                    Gson()
                        .fromJson(jsonInString, LFResponse::class.java)
                        .message
                )
            )
        }
    ...
    fun login(email: String, password: String) = flow {
        ...
        try {
            ...
            emit(
                MyResult.Error(
                    Gson()
                        .fromJson(jsonInString, LFLoginResponse::class.java)
                        .message
                )
            )
        }
    ...
    companion object {
        ...
        fun getInstance(
            ...
            synchronized(AuthRepository::class.java) {
                INSTANCE = INSTANCE ?: AuthRepository(userPreference, apiService)
            }
            return INSTANCE!!
        }
    }
